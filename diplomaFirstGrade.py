...
def get_boundary_equations(data):
    equations = {}
    n = len(data)
    for i in range(n):
        boundary_equations = []
        ...
        equations[i] = boundary_equations
    return equations
...
def plot_diagram(equations):
    ...
    colors = plt.cm.tab10(np.linspace(0, 1, len(keys) * 2))
...
xmin = min(xs)
ymin = min(ys)
xMin, xMax = min(xmin, ymin) - pad, xmax + pad
yMin, yMax = min(xmin, ymin) - pad, ymax + pad
res = 800
...
equations = get_boundary_equations(signal_points)
plot_diagram(equations)
ax.scatter(4, 2, color="black", s=40, marker="X")
ax.plot([5, 5], [0, 5], color="green", linewidth=2)

plt.title("Առաջին կարգի Վորոնովի դիագրամ", fontsize=12)
plt.show()
